<template>
    <v-card elevation="0" color="#f7f7f7">
        ...
        <v-scroll-x-transition>
           <Init :view.sync="view" @updatedState="updateState" @updatedSubState="updateSubState" :total.sync="total" :pedidos.sync="pedidos" :pedidoSelect.sync="pedidoSelect" v-show="view == 1" />
        </v-scroll-x-transition>
        <v-scroll-x-transition>
            <v-card
                :class="
      ...
      if (this.view === 1) {
        ...
        if (savedData) {
          ...
          if(toLoad.pedidoSelect == null)return
            console.log(this.pedidoSelect,toLoad)
