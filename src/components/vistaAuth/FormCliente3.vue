<template>
  <v-form v-model="valid" @submit.prevent>
      ...
      <v-stepper-items>
        <v-stepper-content step="1">
          <v-card class="mb-12" elevation="0">
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn @click="e1=2" color="#005598" :disabled="!valid1" class="white--text">Siguiente</v-btn>
            </v-card-actions>
          </v-card>
        </v-stepper-content>
            ...
            <v-card elevation="0">
              <v-row>
                <v-col
                  ...
                  md="6"
                  sm="12"
                  :offset="$vuetify.breakpoint.smAndDown ? null:3"
                >
                  <v-text-field
                    label="Email"
                    v-model="data.email"
    ...
    <v-snackbar v-model="snackbar" :color="error != null? '#D32F2F':'#2E7D32'" right>
      <div v-if="error==null">
        <v-icon dark>check_circle</v-icon>se ha registrado exitosamente.
      </div>
      <div v-if="error">
        <v-icon dark>calcel</v-icon>
        {{error}}
      </div>
    </v-snackbar>
  </v-form>
