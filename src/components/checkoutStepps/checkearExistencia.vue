        ...
        <v-row justify="center">
            <v-col cols="12" md="6" sm="12" class="pa-5 products-list" v-if="{ pedidoSelect }" >
                ...
                <v-list>
                    ...
                    <v-list-item class="products-row" v-for="(detalle, i) in pedidoSelect.detalles" :key="i">
                        <v-list-item-title>
                            <v-list-item-avatar>
                        ...
                        <v-list-item-title class="product-text">
                            <v-btn class="ma-2 bloked white--text" :loading="loading" :disabled="loading" small v-if="detalle" :color="productsAvaible[i]" @click="loader = 'loading'">
                                {{ detalle.stock }}
                            </v-btn>
                        </v-list-item-title>
                    </v-list-item>
            ...
            <v-col cols="10" md="4" sm="12">
                <v-card class="no-shadow">
                    ...
                    <v-card-text cols="12" md="8" sm="12" class="font-weight-bold subtitle-3" >
                        <v-subheader v-if="stock == null" ><span :style="'color:' + stockNotifier.color">{{ stockNotifier.message}}</span></v-subheader>
                    </v-card-text>
                </v-card>
            </v-col>
        </v-row>

        <v-btn :disabled="this.loading" color="#0f2441" @click="changeView('stepper', 2); bloqueo = true;">
...
const avaible = {message: "Verificacion Exitosa, los productos se encuentran disponibles", color: "green", };
const isOut = {message: "Lo sentimos, se acabo la existencia de susproductos se ha agotado",color: "red", };
const notComplete = { message:"Lo sentimos, se acabo la existencia de algunos de sus Productos, Desea continuar con los disponibles?", color: "red", };
            ...
            return Promise.all(
                ...
                this.pedidoSelect.detalles.map(async (product, i) => {
                    ...
                    if (stock > 0) {
                        ...
                        this.productsAvaible[i] = product.cantidad < stock ? "green" : "red"; 
        ...
        async checkExistence() {
            ...
            this.getCheck().then((checked) => {
                ...
                if (this.disponibilidad === this.pedidoSelect.detalles.length) {
                    ...
                    let suma = 0;
                    this.total = 0;
                    this.pedidoSelect.detalles.map((a) => { suma += a.cantidad < a.stock ? this.checkValue(a.precio) * a.cantidad : this.checkValue(a.precio) * a.stock; });
